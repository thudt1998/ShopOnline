...
@section('breadcrumbs')
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-9">
            <h2><i class="fa fa-list"></i> Danh sách thương hiệu</h2>
        </div>
        <div class="col-lg-3 btn-add">
            <a href="{{route('brand.create')}}" class="btn btn-primary "><i class="fa fa-plus"></i>&nbsp;Thêm thương
                hiệu</a>
        </div>
    </div>
@endsection
                                ...
                                <table class="table table-striped table-bordered table-hover table-zip"
                                    ...
                                    @foreach($brands as $brand)
                                        <tr>
                                            ...
                                            <td>{{$brand->nation}}</td>
                                            <td><span class="article">{{$brand->description}}</span></td>
                                            <td class="text-right footable-visible footable-last-column">
                                                <div class="btn-group">
                                                    <a href="{{route('brand.edit',$brand->id)}}" class="btn-white btn btn-xs">
                                                        <i class="fa fa-edit "></i> Sửa
                                                    </a>
                                                    <a href="#" data-toggle="modal" data-target="#confirm-delete"
                                                       data-href="{{route('brand.delete',$brand->id)}}" class="btn-white btn btn-xs">
